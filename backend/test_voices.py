...
def t_the_patch():
    ...
    # Straight after big-model.patch (its context is big-model's route
    # blocks). Later patches (learning-asks.patch) may follow it.
    check("apply-patches.ps1 applies voices.patch right after big-model.patch",
          "voices.patch" in names
          and names.index("voices.patch") == names.index("big-model.patch") + 1, names[-4:])
