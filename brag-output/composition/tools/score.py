...
def groove_on(t):
    return (5.5 <= t < 11.5) or (14.0 <= t < 18.0)
...
for i in range(int((5.5 - 2.0) / 0.25)):
    at = 2.0 + i * 0.25
    if i % 2 == 0:
...
for i in range(int(DUR / 0.125)):
    ...
    if step % 2 == 1 or at >= 7.5:
        place(music, pan(pluck(arp, bright=1800 + 1400 * (at > 8.0)), 0.5 if step % 4 < 2 else -0.5), at, 0.22)
        place(send, pan(pluck(arp), 0), at, 0.12)

for key in ("listens", "thinks", "speaks"):
    place(drums, pan(clap(), 0), H[key], 1.0)
    place(send, pan(clap(), 0), H[key], 0.6)
place(music, pan(pad(CHORDS[1], 4.0, 0.25), 0), 7.5)

# Montage glitch ticks
for at in H["montage"] + [H["nothing"]]:
    place(ui, load_sfx("interface/glitch_002.ogg"), at, 0.9)

# 10.5 - 11.5: reply streams (key ticks) with a short riser; 11.5 STOP
for k in range(12):
    place(ui, load_sfx(f"keyboard/keypress-{(k * 7) % 30 + 1:03d}.wav"), H["reply"] + 0.1 + k * 0.075, 0.35)
place(fx, pan(riser(1.0, 0.45), 0), 10.5)
place(fx, pan(impact(1.1), 0), stop)
place(ui, load_sfx("impact/impactSoft_heavy_003.ogg"), stop, 0.9)

# 12.0 - 14.0: low drone, then kick build
place(music, pan(pad([55.0, 82.41], 2.1, 0.45), 0), stop_end)
for k in range(4):
    place(drums, pan(kick(0.55 + 0.12 * k), 0), H["ask"] + k * 0.25)
place(fx, pan(riser(1.0, 0.55), 0), H["ask"])
...
place(music, pan(pad(CHORDS[2], 2.5, 0.25), 0), 15.9)

# 18.0 - 21.0: half-time, big sub, riser into the outro
place(fx, pan(impact(0.9), 0), H["private"])
place(bass, pan(braam(41.2, 2.0, 0.55), 0), H["private"])
for k in range(6):
    at = H["private"] + k * 0.5
    place(drums, pan(kick(0.85), 0), at)
    place(bass, pan(sub_note(ROOTS[3] if at < 20 else ROOTS[0], 0.4, 0.8), 0), at)
    place(drums, pan(hat(True), 0.2), at + 0.25, 0.6)
place(drums, pan(clap(), 0), H["local"], 0.8)
place(music, pan(pad(CHORDS[3], 3.0, 0.3), 0), H["private"])
place(fx, pan(riser(2.0, 1.0), 0), 19.0)
for k in range(8):
    place(drums, pan(clap(), 0), 20.0 + k * 0.125, 0.25 + 0.08 * k)
...
place(music, pan(pad([220.0, 261.63, 329.63, 440.0], 4.0, 0.35), 0), H["outro"])
...
for i in range(int(DUR / 0.5)):
    at = i * 0.5
    if groove_on(at) or H["private"] <= at < H["outro"]:
        s = int(at * SR); e = min(N, s + int(0.25 * SR))
        kick_env[s:e] = np.maximum(kick_env[s:e], np.exp(-t_axis(e - s) / 0.09))
duck = 1 - 0.55 * kick_env

mix = drums * 0.8 + bass * 0.8 * duck + music * 0.5 * duck + fx * 0.75 + ui * 0.8
mix += reverb(send + music * 0.4 + fx * 0.2, wet=0.35)

# STOP: everything cuts dead 60 ms after the hit, until the downbeat at 12.0
