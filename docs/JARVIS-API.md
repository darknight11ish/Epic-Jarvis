...
| `/api/memory/status` | GET | `routes.rs:28` | **no** | `{available, db, facts, current, retired, erased, embedder, semantic, vector_search, unembedded, sleep_time}` - `MemoryStore.status()` plus the route's own two. `facts` counts retired ones too; `current` is what is in use; `erased` (since 2026-09-24) counts facts whose words were erased. |
...
- **"Saved automatically"**: newest first, the fact, when, a small "said
  ...
  **"Erase the words"** (2026-09-24, §6 `/api/memory/erase`).
