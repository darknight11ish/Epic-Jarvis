...
## 26. Focus sessions (added 2026-09-25)
...
| `POST /api/focus/start` | `{"minutes": 1-240, "on"?: "<the owner's words, 60 characters>"}` | 200 `{"ok": true, "said", "focus": status}`; 400 minutes out of range; **409** one is already running (`error` says how long is left) | No approval card (26.4). Both apps hold it on a stale link. |
...
| `GET /api/focus/diag` | - | 200 booleans and counts | For "it didn't notice" / "it locked the wrong thing": is the front window readable, is it Jarvis, a browser, is its site readable, deferred, settle ticks, is there an app target / a site target, on target, drifting, is the look thread alive, the knobs. Never what was in front. Neither app shows it; read it on the PC (26.6). |
...
### 26.3 Said or typed - without the model
...
- **The manner setting** ("warm" / "plain") was built at the same time; the
  plain lines are written and tested, but `jarvis_focus.MANNER_SOURCE` is
  not wired to it yet, so the warm lines are used.
- **The "stop everything" hotkey** was built at the same time: its hook
  points are `jarvis_focus.Engine.stop_everything()` (pauses the session and
  drops a waiting line) and `main.js` `stopFocusCallout()`.
