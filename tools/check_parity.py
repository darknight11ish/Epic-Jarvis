...
CLASSIFICATION = {
    ...
    "/api/search": ("ported", "Web search (backend/web-search.patch, the owner's decisions of 2026-09-25; JARVIS-API section 23): the five providers with the PC's own \"why use this one\" lines, which is chosen, whether each is ready, the SearXNG address, \"Ask before every web search\" and Whoogle's reason for being left out. Desktop: Settings, Web search (web_search.rs get_web_search, settings window only). Phone: Brain, Web search (WebSearchPlate.kt, net/WebSearch.kt). An Exa, Tavily or Brave key is typed on the desktop only, straight into Credential Manager - there is no route for one (ARCHITECTURE section 8)."),
    "/api/email/sending": ("ported", "Sending email (backend/email-send.patch, the owner's decision of 2026-09-25 after the Muse audit; JARVIS-API section 26): whether sending is set up, from which address, through which server - the PC's own line, never the password. Desktop: Settings, Sending email (email_sending.rs get_email_sending, settings window only). Phone: Brain, Sending email (EmailSendingPlate.kt, net/EmailSending.kt). Each email itself is an ordinary approval card in both apps (/api/pending), shown in full; the desktop's widget sends an email's Approve to the Jarvis bar."),
    ...
    "/api/memory/status": ("ported", "Memory counts and whether search-by-meaning is on, read-only, in the desktop Memory pane's words. Phone: Brain, What Jarvis remembers (MemoryCountsPlate.kt, net/MemoryCounts.kt)."),
    ...
    "/api/history": ("ported", "Chat history kept on the PC, encrypted (docs/JARVIS-API.md section 18): the switch, why nothing is being kept (if so), how long it is kept, and conversations newest first with Load older. Desktop: the Brain's History tab (brain_history_list). Phone: Brain, Chat history (HistoryScreen.kt, net/ChatLog.kt)."),
