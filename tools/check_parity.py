...
CLASSIFICATION = {
    ...
    "/api/email/sending": ("ported", "Sending email (backend/email-send.patch, the owner's decision of 2026-09-25 after the Muse audit; JARVIS-API section 24): whether sending is set up, from which address, through which server - the PC's own line, never the password. Desktop: Settings, Sending email (email_sending.rs get_email_sending, settings window only). Phone: Mind, Sending email (EmailSendingPlate.kt, net/EmailSending.kt). Each email itself is an ordinary approval card in both apps (/api/pending), shown in full; the desktop's widget sends an email's Approve to the Jarvis bar."),
