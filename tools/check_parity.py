...
CLASSIFICATION = {
    ...
    "/api/search": ("ported", "Web search (backend/web-search.patch, the owner's decisions of 2026-09-25; JARVIS-API section 23): the four providers with the PC's own \"why use this one\" lines, which is chosen, whether each is ready, the SearXNG address, \"Ask before every web search\" and why Whoogle and Brave are left out. Desktop: Settings, Web search (web_search.rs get_web_search, settings window only). Phone: Mind, Web search (WebSearchPlate.kt, net/WebSearch.kt). An Exa or Tavily key is typed on the desktop only, straight into Credential Manager - there is no route for one (ARCHITECTURE section 8)."),
